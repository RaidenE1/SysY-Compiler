from re import A
import sys
from ply import yacc
from tokrules import tokens
from ast_modules import AstNode

def p_CompUnit(p):
    ''' CompUnit : FuncDef '''
    p[0] = p[1]

def p_FuncDef(p):
    ''' FuncDef : FuncType Main LPar RPar Block'''
    p[0] = AstNode('NT', [p[1], p[2], p[3], p[4], p[5]], 'FuncDef')

def p_FuncType(p):
    ''' FuncType : Int '''
    p[0] = p[1]

def p_Block(p):
    ''' Block : LBrace AddBlock RBrace '''
    p[0] = AstNode('NT', [p[1], p[2], p[3]], 'Block')

def p_AddBlock(p):
    ''' AddBlock : BlockItem 
                 | BlockItem AddBlock'''
    if len(p) == 2:
        p[0] = p[1]
    else:
        p[0] = AstNode('NT', [p[1], p[2]], 'AddBlock')

def p_BlockItem(p):
    ''' BlockItem : Decl 
                  | Stmt '''
    p[0] = p[1]

def p_Decl(p):
    ''' Decl : ConstDecl 
             | VarDecl'''
    p[0] = p[1]

def p_ConstDecl(p):
    ''' ConstDecl : Const BType AddConstDef Semicolon '''
    p[0] = AstNode('ConstDecl', [p[1], p[2], p[3], p[4], p[5], p[6], p[7], p[8]], 'ConstDecl')

def p_AddConstDef(p):
    ''' AddConstDef : ConstDef Comma AddConstDef
                    | ConstDef '''
    if len(p) == 4:
        p[0] = AstNode('NT', [p[1], p[2], p[3]], 'ConstDef')
    else:
        p[0] = p[1]

def p_ConstDef(p):
    ''' ConstDef : IDENT Assign ConstInitVal '''
    p[0] = AstNode('NT', [AstNode('T', None, 'Ident', p[1], True), p[2], p[3]], 'ConstDef')

def p_BType(p):
    ''' BType : Int'''
    p[0] = AstNode('T', None, 'BType', p[1])

def p_ConstInitVal(p):
    ''' ConstInitVal : ConstExp '''
    p[0] = p[1]

def p_ConstExp(p):
    ''' ConstExp : AddExp '''
    p[0] = p[1]
    
def p_VarDecl(p):
    ''' VarDecl : BType AddVarDef Semicolon '''
    p[0] = AstNode('NT', [p[1], p[2], p[3]], 'VarDecl')

def p_AddVarDef(p):
    ''' AddVarDef : VarDef Comma AddVarDef
                  | VarDef '''
    if len(p) == 4:
        p[0] = AstNode('AddVarDef', [p[1], p[2], p[3]], 'AddVarDef')
    else:
        p[0] = p[1]

def p_VarDef(p):
    ''' VarDef  : IDENT
                | IDENT Assign InitVal '''
    if len(p) == 2:
        p[0] = AstNode('T', None, 'Ident', p[1], False)
    else:
        p[0] = AstNode('NT', [AstNode('T', None, 'Ident', p[1], False), p[2], p[3]], 'VarDef')

def p_InitVal(p):
    ''' InitVal : Exp '''
    p[0] = p[1]

def p_Stmt(p):
    ''' Stmt : Return Exp Semicolon 
             | LVal Assign Exp Semicolon
             | Exp Semicolon '''
    if len(p) == 4:
        p[0] = AstNode('NT', [p[1], p[2], p[3]], 'return')
    elif len(p) == 5:
        p[0] = AstNode('NT', [p[1], p[2], p[3], p[4]], 'VarAssign')
    else:
        p[0] = AstNode('NT', [p[1], p[2]], 'Single')

def p_LVal(p):
    ''' LVal : IDENT '''
    p[0] = AstNode('T', None, p[1])
     
def p_Exp(p):
    ''' Exp : AddExp '''
    p[0] = p[1]

def p_AddExp(p):
    ''' AddExp : MulExp 
               | AddExp Plus MulExp
               | AddExp Minus MulExp '''
    if len(p) == 2:
        res_val = str(p[1].val)
    else:
        res_val = str(p[1].val) + ' ' + p[2] + ' ' + str(p[3].val)
    p[0] = AstNode('T', None, 'AddExp', int(eval(res_val)))

def p_MulExp(p):
    ''' MulExp : UnaryExp 
               | MulExp Div UnaryExp
               | MulExp Mult UnaryExp
               | MulExp Mod UnaryExp'''
    if len(p) == 2:
        res_val = str(p[1].val)
    elif p[2] == '/':
        res_val = str(p[1].val) + ' // ' + str(p[3].val)
    elif p[2] == '%':
        if eval(str(p[1].val)) < 0: 
            res_val = str(- (abs(eval(str(p[1].val)) % abs(eval(str(p[3].val))))))
        else:
            res_val = str( (abs(eval(str(p[1].val)) % abs(eval(str(p[3].val))))))
    else: 
        res_val = str(p[1].val) + ' ' + p[2] + ' ' + str(p[3].val)
    p[0] = AstNode('T', None, 'MulExp', int(eval(res_val)))

def p_UnaryExp(p):
    ''' UnaryExp : PrimaryExp 
                 | Plus UnaryExp
                 | Minus UnaryExp '''
    if len(p) == 2:
        res_val = str(p[1].val)
    else:
        res_val = p[1] + ' ' + str(p[2].val)
    p[0] = AstNode('T', None, 'UnaryExp', int(eval(res_val)))


def p_PrimaryExp(p):
    ''' PrimaryExp : LPar Exp RPar 
                   | Number 
                   | LVal '''
    if len(p) == 2:
        res_val = str(p[1].val)
    else:
        res_val = p[1] + ' ' + str(p[2].val) + ' ' + p[3]
    p[0] = AstNode('T', None, 'PriExp', int(eval(res_val)))

def p_Number(p):
    ''' Number  : DECIMAL 
                | OCTAL 
                | HEXADECIMAL '''
    res_val = str(p[1])
    p[0] = AstNode('T', None, 'Number', int(eval(res_val)))

def p_error(p):
    sys.exit(1)
    # print("Error de sintaxis", p)
    # print("Error en linea: "+ str(p.lineno))

def run_parser(text, lexer):
    parser = yacc.yacc(start = 'CompUnit')
    result = parser.parse(text, lexer)
    # print(result)
    return result


